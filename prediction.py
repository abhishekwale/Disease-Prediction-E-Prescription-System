...
import os
import ast  # Add ast import for literal_eval
...
def log_prediction(user_email, symptoms, predicted_disease, pincode='Not provided'):
    init_log_file()
    log_entry = {
...
def get_prediction_logs(user_email=None):
    init_log_file()
    with open(LOG_FILE, 'r') as f:
        logs = json.load(f)
    if user_email:
        return [log for log in logs if log['user_email'] == user_email]
    return logs
...
def process_symptoms(symptoms, user_email, pincode='Not provided'):
    if not symptoms:
        return None, "Please select at least one symptom."
    ...
    for symptom in symptoms:
        ...
        if corrected:
            ...
            return None, f"Symptom '{symptom}' not recognized."
    ...
    # Log prediction with pincode
    log_prediction(user_email, corrected_symptoms, predicted_disease, pincode)
    ...
    return {
        ...
        'workout': workout.values[0] if not workout.empty else []
    }, None